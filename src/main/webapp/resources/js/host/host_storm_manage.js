	...
	$.ajax({
		...
		success : function(data) {
			...
			layer.open({
				...
				area : [ '550px', '340px' ],
				content : $("#editStormHost"),
				btn : [ '确定', '取消' ],
				btn1 : function(index, layero) {// 确定按钮回调
					if (form.isValidator(formObj)) {
						$.ajax({
							"url" : webpath + "/hostStorm/editStorm",
							"type" : "POST",
							dataType : "json",
							data : form.serializeJson(formObj),
							success : function(data) {
