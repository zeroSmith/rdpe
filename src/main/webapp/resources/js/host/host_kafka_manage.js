	...
	$.ajax({
		...
		success : function(data) {
			...
			layer.open({
				...
				area : [ '550px', '340px' ],
				content : $("#editKafkaHost"),
				btn : [ '确定', '取消' ],
				btn1 : function(index, layero) {// 确定按钮回调
					if (form.isValidator(formObj)) {
						$.ajax({
							"url" : webpath + "/hostKafka/editKafka",
							"type" : "POST",
							dataType : "json",
							data : form.serializeJson(formObj),
							success : function(data) {
								if(data==true){
									...
									layer.msg('更新失败', {time: 2000, icon:6});
								}
							}
						});
					}
