...
function addEvent() {
	...
	// 获得master主机
	$.ajax({
		url : webpath + "/hostSpark/getMaster",
		type : "POST",
		dataType : "json",
	...
	layer.open({
		...
		btn1 : function(index, layero) {// 确定按钮回调
			...
			if (form.isValidator(formObj)) {
				$.ajax({
					url : webpath + "/event/spark/add",
					type : "POST",
					dataType : "json",
					data : {
						eventSparkJson : form.serializeStr(formObj),
...
function remove(id) {
	...
	// 按钮
		...
		$.ajax({// 初始化组织机构树
			"url" : webpath + "/event/spark/remove",
			"type" : "POST",
			dataType : "json",
			data : {
				id : id
...
function code(id) {
	$.ajax({
		"url" : webpath + "/event/spark/gencode",
		"type" : "POST",
		dataType : "json",
		data : {
			id : id
...
function publish(id) {
	// 自定义事件及外部事件都需要发布
	$.ajax({
		"url" : webpath + "/event/spark/publish",
		"type" : "POST",
		dataType : "json",
		data : {
			id : id
...
function processEvent(id) {
	window.location.href = webpath + "/event/spark/process?id=" + id;
}
